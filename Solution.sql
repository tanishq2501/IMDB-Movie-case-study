...
SELECT
    SUM(CASE WHEN id IS NULL THEN 1 ELSE 0
       END) AS id_null_count,
    SUM(CASE WHEN title IS NULL THEN 1 ELSE 0
        END) AS title_null_count,
    SUM(CASE WHEN YEAR IS NULL THEN 1 ELSE 0
        END) AS year_null_count,
    SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0
        END) AS date_published_null_count,
    SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0
    ...
    SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0
...
SELECT COUNT(id) AS No_of_movie_produced
FROM movie
WHERE YEAR = 2019 
	  AND 
...
SELECT genre, COUNT(m.id) AS No_of_movie_produced_overall
FROM genre g
     INNER JOIN movie m 
     ON g.movie_id = m.id
GROUP BY genre
ORDER BY No_of_movie_produced_overall desc
limit 1;
...
WITH movie_belonging_to_only_one_genre AS 
(
 SELECT movie_id
 FROM genre g
      INNER JOIN movie m 
...
SELECT COUNT(*) AS movie_with_only_one_genre
FROM movie_belonging_to_only_one_genre;
...
SELECT COUNT(id) AS No_of_movie_released
FROM movie m
	INNER JOIN ratings r 
    ON m.id = r.movie_id
...
SELECT
      ...
      total_italian_votes,
      (
        CASE WHEN total_german_votes > total_italian_votes THEN 'YES' ELSE 'NO'
        END
        ) 
        AS is_german_votes_greater_than_italian_votes
FROM german_movies_votes, italian_movies_votes;
...
SELECT * 
FROM find_rank WHERE movie_rank <=5;
...
( 
	SELECT d.name_id, NAME, d.movie_id, duration, r.avg_rating, total_votes, m.date_published, Lead(date_published,1) 
